...
export const initializeNursingSampleData = () => {
  ...
  // Define event types (scheduled activities)
  ...
  // Map event types
  const mapEventType = (csvType: string): 'lecture' | 'clinical' | 'lab' | 'exam' | 'simulation' | 'review' => {
    switch(csvType.toLowerCase()) {
      case 'lecture':
        return 'lecture';
      case 'clinical':
  ...
  rawData.forEach(item => {
    // Skip TBD dates
    if (item.date === 'TBD') return;
    
    // Find the course
    const course = courses.find(c => c.code === item.course);
    if (!course) return;
    
    // Check if it's an event or a task
    if (eventTypes.includes(item.type)) {
      // It's an event - add to events
      const startTime = new Date(item.date);
      const duration = parseDuration(item.duration) || 2; // Default 2 hours if no duration
      
      // Set appropriate start times based on type
      if (item.type === 'Clinical') {
        startTime.setHours(7, 0, 0); // Clinical days start at 7 AM
      } else if (item.type === 'Exam') {
        startTime.setHours(9, 0, 0); // Exams at 9 AM
      } else if (item.type === 'Lecture') {
        startTime.setHours(9, 0, 0); // Lectures at 9 AM
      } else if (item.type === 'Lab' || item.type === 'Simulation') {
        startTime.setHours(13, 0, 0); // Labs/Simulations at 1 PM
      }
      ...
      store.addEvent({
        ...
        startTime,
        endTime,
        description: `${item.type}${duration ? ` (${item.duration})` : ''}`
      });
      
      eventsCount++;
    } else {
      // It's a task - add to tasks
      const dueDate = new Date(item.date + 'T23:59:00');
      const duration = parseDuration(item.duration);
      const taskType = mapTaskType(item.type);
      const complexity = estimateComplexity(item.type, duration);
      ...
      store.addTask({
        ...
        dueDate,
        complexity,
        estimatedHours: duration || 0,
        isHardDeadline: false,
        bufferPercentage: 20,
        status: 'not-started',
        description: `Type: ${item.type}${duration ? `, Duration: ${item.duration}` : ''}`
      });
  ...
  console.log(`Loaded ${tasksCount} tasks and ${eventsCount} events from nursing schedule`);
};
